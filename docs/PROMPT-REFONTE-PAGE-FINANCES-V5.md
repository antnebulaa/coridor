...
**Données existantes dans le simulateur d'investissement :** Le simulateur (`/simulateur-investissement`) collecte déjà prix d'achat, mode d'acquisition, financement (apport, type de prêt, durée, taux, assurance emprunteur), loyer, charges récupérables, taxe foncière, vacance locative, revenus saisonniers, TMI, régime fiscal. Les résultats calculent rendement brut/net/net-net, TRI, comparatif vs Livret A/Bourse/Assurance-vie, impact fiscal, comparaison des régimes, plus-value à la revente. **Ces données et calculs doivent être réutilisés sur la page /finances** — ne pas recréer de modèles ou de formules. Si le proprio a déjà fait une simulation pour un bien qu'il possède, on peut en extraire prix d'achat, données d'emprunt, et projections.

**Données à ajouter au modèle Property (si pas déjà présentes) :**
- `purchasePrice: Float?` — prix d'achat du bien
- `acquisitionMode: String?` — "ACHAT" | "DONATION" | "HERITAGE"
- `purchaseDate: DateTime?` — date d'achat
- `loanAmount: Float?` — capital emprunté
- `loanMonthlyPayment: Float?` — mensualité
- `loanEndDate: DateTime?` — date de fin de prêt
- `loanRate: Float?` — taux d'intérêt
- `estimatedValue: Float?` — valeur estimée actuelle (saisie manuelle ou future intégration DVF)

Si ces champs existent déjà via le simulateur, les réutiliser directement. L'idée est que le proprio qui a fait une simulation pour son bien existant voit automatiquement les données remonter sur la page /finances.

**Prototype de référence :** le fichier `finances-v5-smart.jsx` dans le projet — c'est la maquette visuelle de référence. Ne pas copier le code tel quel mais respecter la structure, la hiérarchie et le design system.
...
├──────────────────────────────────────────────┤
│  COÛT PAR BIEN (timeline financière)         │
│  [Bien 1 · +16 079 € · 12 mois cliquables]  │
│  [Bien 2 · −3 408 € · 12 mois rouges]       │
│  Chaque mois : vert = revenu, rouge = coût   │
│  Clic sur un mois → détail (loyer perdu +    │
│  charges fixes qui continuent de courir)      │
│  + N autres biens → Voir tout                │
├──────────────────────────────────────────────┤
...
**Mission :** Créer tous les composants de la page finances. Design fintech unifié, mobile-first, animations stagger, Open Doodles sur les insights. S'inspirer strictement du prototype `finances-v5-smart.jsx`.
...
- `components/finances/PropertyCostSection.tsx` — **Nouveau**
- `components/finances/PropertyTimeline.tsx` — **Nouveau**
...
interface FinancialReport {
  ...
  properties: {
    ...
    annualResult: number;           // total revenu - total coût sur l'année (peut être négatif)
    monthlyTimeline: {              // 12 entrées, une par mois
      month: number;                // 1-12
      revenue: number;              // loyer encaissé ce mois (0 si vacant)
      cost: number;                 // coût total si vacant (loyer perdu + charges fixes)
      lostRent: number;             // loyer qu'on aurait dû percevoir
      fixedCosts: number;           // charges qui courent même sans locataire (assurance, taxe foncière mensuelle, copro...)
    }[];
  }[];
...
**Réutilisation des données du simulateur d'investissement :**

Le simulateur existant (`/simulateur-investissement`) utilise déjà des modèles et calculs pour : rendement brut/net/net-net, TRI 20 ans, comparatif vs Livret A (3%)/Bourse (S&P 500 10.5%)/Assurance-vie (2%), plus-value brute et nette à la revente, impact fiscal par régime (micro-foncier, réel, LMNP réel, micro-BIC, SCI IS), coût de l'emprunt (intérêts + assurance), cashflow cumulé.

**Ne pas recréer ces formules.** Identifier les fonctions de calcul existantes dans le codebase du simulateur et les importer/réutiliser dans `getFinancialReport.ts`. En particulier :
- Calcul du rendement net = réutiliser la formule du simulateur
- Comparatif vs Livret A / Bourse = réutiliser la logique existante
- Plus-value nette après impôts = réutiliser le calcul de l'onglet "Revente"
- Impact fiscal = réutiliser la comparaison des régimes

Si le proprio a fait une simulation sauvegardée pour un bien qu'il possède, croiser les données (par adresse ou par lien explicite simulation ↔ propriété).
...
### PropertyCostSection.tsx

Section "Vos biens en {year}" avec sous-titre "Chaque mois : ce qu'il a rapporté ou coûté".

Chaque bien est une card individuelle contenant :
- En-tête : nom du bien + badge "Vacant" si pas de locataire + résultat annuel à droite (vert si positif, rouge si négatif)
- Sous-titre : adresse + locataire + badge date fin bail (si applicable)
- **Timeline cliquable** : 12 segments (un par mois), chacun est un bouton
  - **Mois avec revenu** : fond `bg-emerald-100 text-emerald-700`, hover `bg-emerald-200`
  - **Mois vacant / à perte** : fond `bg-red-50 text-red-400`, hover `bg-red-100`
  - Mois sélectionné : couleur pleine (`bg-emerald-500 text-white` ou `bg-red-400 text-white`)
  - Label du mois (J, F, M...) centré dans chaque segment
- **Panneau de détail** (apparaît sous la timeline au clic sur un mois) :
  - Mois avec revenu : "+729 € · Loyer encaissé"
  - Mois vacant : "−850 € · Loyer perdu : 729 € + charges fixes : 121 €"
  - Le proprio comprend immédiatement qu'un mois vide n'est pas neutre

Maximum 2-3 biens affichés par défaut. Au-delà : bouton "+ N autres biens · Voir tout →".

**Impact psychologique :** un bien 100% vacant sur l'année, c'est 12 rectangles rouges avec un total "−3 408 €". C'est beaucoup plus parlant que "Taux d'occupation : 0%". Ça pousse à l'action.

### PropertyTimeline.tsx

Props : `months: { revenue: number; cost: number; lostRent: number; fixedCosts: number }[]`, `currentMonth?: number` (0-11, le mois en cours)

- 12 boutons en flex row, gap-[3px], hauteur h-7, `rounded-md`
- Chaque bouton affiche la lettre du mois (J, F, M...)
- **Le mois en cours est sélectionné par défaut** (pas besoin de cliquer), avec son panneau de détail ouvert. Si on est en mars, le proprio voit immédiatement "+729 € — Loyer encaissé" pour mars sans toucher à rien
- Le mois en cours a un indicateur visuel supplémentaire : un petit point/dot en dessous ou un contour légèrement plus épais, pour qu'on le repère même quand on navigue sur un autre mois
- `onClick` → toggle un panneau de détail en dessous (fade-in)
- Le panneau montre le montant + la décomposition (loyer perdu + charges fixes)
- Mois futurs (après le mois en cours) : style atténué (`opacity-50`), pas de données réelles — afficher "À venir" si cliqué
...
5. `PropertyCostSection` (stagger delay 350ms)
...
| `components/finances/PropertyCostSection.tsx` | 2 | Section timeline financière par bien |
| `components/finances/PropertyTimeline.tsx` | 2 | Timeline 12 mois cliquable (revenu/coût) |
...
### Agent 1
- [ ] Formules rendement/TRI/plus-value réutilisées du simulateur d'investissement (pas recréées)
...
- [ ] Timeline par bien : chaque mois a revenue OU cost (pas les deux)
- [ ] Mois vacant : cost = lostRent (loyer du bail ou estimation) + fixedCosts (assurance PNO mensuelle + taxe foncière /12 + copro /12)
- [ ] annualResult par bien = somme des revenues - somme des costs
...
- [ ] PropertyTimeline : mois en cours sélectionné par défaut avec détail ouvert
- [ ] PropertyTimeline : mois futurs grisés (opacity-50, non cliquables)
- [ ] PropertyTimeline : dot indicateur sous le mois en cours quand un autre mois est sélectionné
- [ ] PropertyTimeline : 12 segments cliquables, vert = revenu, rouge = coût
- [ ] PropertyTimeline : clic sur un mois → détail avec décomposition (loyer perdu + charges fixes)
- [ ] PropertyTimeline : un bien 100% vacant = 12 segments rouges + total négatif rouge
- [ ] PropertyCostSection : résultat annuel par bien en haut à droite (vert/rouge)
- [ ] PropertyCostSection : max 2-3 biens visibles, "+ N autres" si plus
