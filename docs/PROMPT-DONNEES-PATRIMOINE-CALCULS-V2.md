...
La première question n'est PAS le prix. C'est le **mode d'acquisition** — parce qu'un bien hérité à 0€ n'est pas un bien qui ne vaut rien. C'est un bien dont la valeur d'acquisition fiscale est celle déclarée dans la succession.

| Priorité | Champ(s) | Ce que ça débloque | Flow |
|----------|----------|-------------------|------|
| 1 | `acquisitionMode` + `acquisitionValue` + `purchaseDate` | Rendement net, comparatif placements, plus-value brute, abattements | Bottom sheet en 2 étapes (voir ci-dessous) |
| 2 | `estimatedCurrentValue` | Patrimoine total, plus-value, equity | "Combien vaut votre bien aujourd'hui ?" |
| 3 | `loanAmount` + `loanRate` + `loanEndDate` | Equity nette, capital restant dû, intérêts déductibles 2044 | "Renseignez votre crédit pour voir votre equity nette" |

**Logique d'affichage :** ne montrer l'invitation N que si les données N-1 sont remplies pour au moins un bien. Ne pas montrer 3 invitations à la fois.

**Note :** la priorité 1 regroupe 3 champs en un seul flow de 2 étapes (pas 3 invitations séparées). C'est rapide mais complet.
...
interface DataInviteCardProps {
  ...
  propertyId: string;
  propertyName: string;
  fieldToCollect: 'acquisition' | 'estimatedValue' | 'loan';
  onComplete: () => void;
}
```
...
**Messages d'invitation par priorité :**

| Priorité | Titre | Description |
|----------|-------|-------------|
| 1 (acquisition) | "D'où vient ce bien ?" | "Achat, héritage ou donation — renseignez l'origine pour découvrir votre rendement réel et comparer avec le Livret A" |
| 2 (valeur estimée) | "Combien vaut votre bien aujourd'hui ?" | "Découvrez votre plus-value et votre patrimoine net" |
| 3 (emprunt) | "Avez-vous un crédit sur ce bien ?" | "Renseignez votre emprunt pour voir votre equity nette et vos intérêts déductibles" |

### PropertyDataSheet.tsx

Bottom sheet adaptatif selon le `fieldToCollect` :

**fieldToCollect = "acquisition" — Flow en 2 étapes :**

**Étape 1 : Mode d'acquisition**
```
┌──────────────────────────────────────┐
...
Trois boutons pleine largeur, un seul tap, pas de sélecteur ni de dropdown.

**Étape 2 : Valeur + Date (s'adapte au mode choisi)**
...
```
┌──────────────────────────────────────┐
│  Valeur déclarée dans la succession  │
│  [ 180 000                     ] €   │
│                                      │
│  C'est la valeur vénale estimée      │
│  au jour du décès, inscrite dans     │
│  l'acte du notaire.                  │
│                                      │
...
```
┌──────────────────────────────────────┐
│  Valeur déclarée dans la donation    │
│  [ 200 000                     ] €   │
│                                      │
│  C'est la valeur vénale au jour      │
...
**Points clés :**
- Le label du champ montant s'adapte : "Prix d'achat" vs "Valeur déclarée dans la succession" vs "Valeur déclarée dans la donation"
- Un petit texte explicatif pour héritage/donation aide le proprio à trouver le bon chiffre ("inscrite dans l'acte du notaire")
- La date s'adapte : "Date d'achat" vs "Date de la succession" vs "Date de la donation"
- Le format date = mois/année (pas besoin du jour exact)
- Après validation → `PATCH /api/properties/[id]` avec `acquisitionMode`, `purchasePrice` (= valeur d'acquisition quelle que soit l'origine), `purchaseDate`

**fieldToCollect = "estimatedValue" :**
```
┌──────────────────────────────────────┐
│  Appartement Lumineux                │
│                                      │
...
│  [ 280 000                     ] €   │
│                                      │
│  Estimation personnelle ou basée     │
...
Si "Non, pas de crédit" → valider immédiatement, aucun emprunt pour ce bien. L'equity nette = valeur estimée (pas de dette).

**Champs sauvegardés par type :**

| fieldToCollect | Champs Property mis à jour |
|---------------|---------------------------|
| `acquisition` | `acquisitionMode`, `purchasePrice`, `purchaseDate` |
| `estimatedValue` | `estimatedCurrentValue`, `estimatedValueDate` (= now) |
| `loan` | `loanAmount`, `loanRate`, `loanEndDate`, `loanBank` (ou tous null si "pas de crédit") |
...
function calculateNetCapitalGain(property: Property): {
  ...
  // La date de détention dépend du mode d'acquisition :
  // - ACHAT : date de l'acte de vente
  // - HERITAGE : date du décès (= purchaseDate dans notre modèle)
  // - DONATION : date de l'acte de donation (= purchaseDate dans notre modèle)
  // Dans tous les cas, purchaseDate contient la bonne date.
  const holdingYears = differenceInYears(new Date(), property.purchaseDate);

  // Frais d'acquisition :
  // - ACHAT : frais de notaire réels ou forfait 7,5% du prix
  // - HERITAGE : droits de succession (forfait 7,5% admis par l'administration)
  // - DONATION : droits de donation (forfait 7,5% admis par l'administration)
  const notaryFees = Math.round(property.purchasePrice * 0.075);

  // Travaux : réels (agrégés depuis Dépenses & Charges) ou forfait 15% si > 5 ans
  // On prend le plus avantageux
  const worksExpenses = await getWorksExpensesForProperty(property.id);
  const worksForfait = holdingYears >= 5 ? Math.round(property.purchasePrice * 0.15) : 0;
  const worksCost = Math.max(worksExpenses, worksForfait);
  ...
  return {
    ...
    holdingYears,
    acquisitionMode: property.acquisitionMode || 'ACHAT',
  };
}
...
### Agent 2
- [ ] Première invitation = "D'où vient ce bien ?" (mode d'acquisition), PAS "Combien avez-vous payé ?"
- [ ] Bottom sheet étape 1 : 3 boutons (Achat / Héritage / Donation), un seul tap
- [ ] Bottom sheet étape 2 : labels adaptés au mode choisi ("Prix d'achat" vs "Valeur déclarée dans la succession" vs "Valeur déclarée dans la donation")
- [ ] Bottom sheet étape 2 : texte explicatif pour héritage/donation ("inscrite dans l'acte du notaire")
- [ ] Bottom sheet étape 2 : date adaptée ("Date d'achat" vs "Date de la succession" vs "Date de la donation")
- [ ] Sauvegarde : `acquisitionMode` + `purchasePrice` (= valeur d'acquisition quel que soit le mode) + `purchaseDate`
- [ ] Invitation emprunt : option "Non, pas de crédit" qui valide immédiatement sans champs
- [ ] Une seule invitation à la fois (pas 3 formulaires)
