...
// ─── Financial Timeline per property ───
// Each month shows: green = revenue earned, red = cost of vacancy
function PropertyTimeline({ months, currentMonth = 2 }) {
  const [selected, setSelected] = useState(currentMonth);
  const labels = ["J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D"];
  const now = currentMonth; // March = 2
        ...
        {months.map((m, i) => {
          ...
          return (
            <button
              ...
              className={`flex-1 h-7 rounded-md text-[8px] font-bold flex items-center justify-center transition-all duration-200 relative ${
                isFuture
                  ? "bg-neutral-50 text-neutral-300 cursor-default"
                  : m.revenue > 0
                    ? selected === i ? "bg-emerald-500 text-white" : "bg-emerald-100 text-emerald-700 hover:bg-emerald-200"
                    : selected === i ? "bg-red-400 text-white" : "bg-red-50 text-red-400 hover:bg-red-100"
              }`}
              disabled={isFuture}
            >
              {labels[i]}
              {i === now && selected !== i && (
                <span className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-neutral-900" />
              )}
            </button>
          );
        })}
      </div>
      {selected !== null && !( selected > now) && (
        <div className="mt-2 bg-neutral-50 rounded-xl px-3 py-2.5 animate-fadeIn">
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-neutral-500">
              {["Janvier", "Février", "Mars", "Avril", "Mai", "Juin", "Juillet", "Août", "Septembre", "Octobre", "Novembre", "Décembre"][selected]}
              {selected === now && <span className="ml-1.5 text-[9px] font-semibold text-neutral-400">· En cours</span>}
            </span>
            {months[selected].revenue > 0 ? (
              <span className="text-xs font-bold text-emerald-600 tabular-nums">+{fmt(months[selected].revenue)} €</span>
            ) : (
              <span className="text-xs font-bold text-red-500 tabular-nums">−{fmt(months[selected].cost)} €</span>
            )}
          </div>
          {months[selected].revenue > 0 ? (
            <p className="text-[10px] text-neutral-400 mt-0.5">Loyer encaissé</p>
          ) : (
        ...
        <Reveal delay={350}>
          <div className="mb-6">
            <div className="flex items-end justify-between mb-3">
              <div>
                <p className="text-[10px] text-neutral-400 uppercase tracking-wider font-semibold">Vos biens en {year}</p>
                <p className="text-xs text-neutral-400 mt-0.5">Chaque mois : ce qu'il a rapporté ou coûté</p>
              </div>
            </div>
              ...
              {[
                {
                  ...
                  totalPositive: true,
                  months: [
                    { revenue: 729, cost: 0, lostRent: 0, fixedCosts: 0 },
                    { revenue: 729, cost: 0, lostRent: 0, fixedCosts: 0 },
                    { revenue: 729, cost: 0, lostRent: 0, fixedCosts: 0 },
                    { revenue: 729, cost: 0, lostRent: 0, fixedCosts: 0 },
                    { revenue: 729, cost: 0, lostRent: 0, fixedCosts: 0 },
                    ...
                    { revenue: 729, cost: 0, lostRent: 0, fixedCosts: 0 },
                    { revenue: 729, cost: 0, lostRent: 0, fixedCosts: 0 },
                    { revenue: 0, cost: 1521, lostRent: 729, fixedCosts: 792 },
                    { revenue: 729, cost: 0, lostRent: 0, fixedCosts: 0 },
                    { revenue: 729, cost: 0, lostRent: 0, fixedCosts: 0 },
                  ]
                },
                {
                  name: "Studio Rivoli",
                  addr: "10 Rue de Rivoli",
                  tenant: null,
                  bailEnd: null,
                  bailMonths: null,
                  total: "−3 408 €",
                  totalPositive: false,
                  months: Array(12).fill({ revenue: 0, cost: 284, lostRent: 200, fixedCosts: 84 })
                },
              ].map((p, i) => (
                  ...
                  <div className="flex items-start justify-between mb-1">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {!p.tenant && (
                          <span className="text-[9px] font-semibold text-red-500 bg-red-50 px-1.5 py-0.5 rounded-full flex-shrink-0">Vacant</span>
                        )}
                      </div>
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        <span className="text-[10px] text-neutral-400">{p.addr}</span>
                    ...
                    <span className={`text-sm font-bold tabular-nums flex-shrink-0 ml-3 ${p.totalPositive ? "text-emerald-600" : "text-red-500"}`}>
                      {p.total}
                    </span>
                  </div>

                  <PropertyTimeline months={p.months} />
                </div>
              ))}

              {/* More properties */}
              <button className="w-full py-3 text-center text-xs font-semibold text-neutral-500 hover:text-neutral-700 transition-colors bg-white border border-neutral-100 rounded-2xl hover:border-neutral-200">
                + 4 autres biens · Voir tout →
              </button>
            </div>
          </div>
        </Reveal>
