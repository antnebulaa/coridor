...
const RegularizationModal: React.FC<RegularizationModalProps> = ({
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            getEligibleLeases(propertyId)
                ...
                .catch(() => toast.error('Erreur chargement locataires'))
    ...
    const onNext = async () => {
        if (step === STEPS.SELECT) {
            ...
            try {
                ...
                toast.error('Erreur calcul régularisation');
            } finally {
                setIsLoading(false);
            }
            ...
            try {
                ...
                toast.success('Régularisation validée !');
    ...
    const actionLabel = useMemo(() => {
        if (step === STEPS.SELECT) return 'Calculer';
        if (step === STEPS.PREVIEW) return 'Valider et Clôturer';
        return 'Terminer';
    }, [step]);
    ...
    let bodyContent = (
        <div className="flex flex-col gap-4">
            <Heading
                ...
                subtitle="Sélectionnez le locataire et l'année concernée."
            />
            <div className='flex flex-col gap-4'>
                <label className="block text-sm font-medium text-gray-700">Locataire / Bail</label>
                <select
                ...
                <label className="block text-sm font-medium text-gray-700">Année Fiscale</label>
                <select
                    value={selectedYear}
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="bg-neutral-50 p-4 rounded-xl space-y-3">
                    <div className="flex justify-between items-center text-sm">
                        <span className="text-neutral-500">Charges Récupérables (Réelles)</span>
                        <span className="font-semibold text-neutral-900">{real.toFixed(2)} €</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                        <span className="text-neutral-500">Provisions perçues</span>
                        <span className="font-semibold text-green-600">{provisions.toFixed(2)} €</span>
                    </div>
                    <hr />
                    <div className="flex flex-col gap-1">
                        <span className="text-sm font-medium text-neutral-600">Solde de régularisation</span>
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-neutral-500">reste à régulariser</span>
                            <span className={`text-xl font-medium ${balance > 0 ? "text-red-500" : "text-green-600"}`}>
                                {balance > 0 ? `${balance.toFixed(2)} € ` : `À rembourser ${Math.abs(balance).toFixed(2)} €`}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <h4 className="font-medium mb-4 text-sm text-neutral-500">Détail des dépenses incluses ({previewData?.expenses?.length})</h4>
                ...
                <div className="text-xs text-neutral-400 bg-blue-50 p-2 rounded text-center">
                    En validant, ces dépenses seront verrouillées et un historique sera créé.
                </div>
            </div>
        );
    }
        ...
        bodyContent = (
            <div className="flex flex-col items-center justify-center gap-6 py-8">
                ...
                <div className="text-center">
                    <h3 className="text-xl font-bold mb-2">Régularisation Enregistrée !</h3>
                    <p className="text-neutral-500 text-sm max-w-xs mx-auto mb-2">
                        Le décompte a été généré et les dépenses ont été verrouillées.
                    </p>

                    {sendingStatus === 'sending' && (
                        <p className="text-blue-500 text-xs flex items-center justify-center gap-2">
                            <Loader2 className="animate-spin" size={14} /> Envoi au locataire...
                        </p>
                    )}
                    {sendingStatus === 'sent' && (
                        <p className="text-green-600 text-xs flex items-center justify-center gap-2">
                            <CheckCircle size={14} /> Envoyé via la messagerie
                        </p>
                    )}
                    {sendingStatus === 'error' && (
                        <p className="text-red-500 text-xs">
                            Échec de l'envoi automatique.
                        </p>
                    )}
                </div>
