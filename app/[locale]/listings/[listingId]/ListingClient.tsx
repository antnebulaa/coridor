...
const ListingClient: React.FC<ListingClientProps> = ({
    ...
    const onContactHost = useCallback(() => {
        ...
        if (isOwner) {
            return toast.error(t('errors.selfContact'));
        }
    ...
    const onApply = useCallback(() => {
        ...
        if (isOwner) {
            return toast.error(t('errors.selfApply'));
        }
                        ...
                        <div className="order-first mb-10 md:order-last md:col-span-3">
                            <div className="bg-card border border-border overflow-hidden rounded-xl p-4 flex flex-col gap-4 sticky top-28">
                                ...
                                <div className="text-muted-foreground font-light">
                                    {t('status.contactText')}
                                </div>
                                {!isOwner && (
                                    <>
                                        <Button
                                            label={t('actions.contactHost')}
                                            onClick={onContactHost}
                                            disabled={isLoading}
                                        />
                                        <hr />
                                        <div className="text-muted-foreground font-light text-sm">
                                            {t('status.completeDossierText')}
                                        </div>
                                        <Button
